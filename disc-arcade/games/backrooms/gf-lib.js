...
function gameplayStart() {
  try {
    ...
    if (platform === 'yandex' && window.ysdk && window.ysdk.features && window.ysdk.features.GameplayAPI && window.ysdk.features.GameplayAPI.start) {
...
function gameplayStop() {
  try {
    ...
    if (platform === 'yandex' && window.ysdk && window.ysdk.features && window.ysdk.features.GameplayAPI && window.ysdk.features.GameplayAPI.stop) {
  ...
  return new Promise(function(rawOk) {
    ...
    try {
      ...
      if (platform === 'yandex' && window.ysdk && window.ysdk.adv) {
        var opened = false, rewarded = false, openTimer = null, maxTimer = null;
        ...
        var onOpen = function () {
          ...
          if (openTimer) { clearTimeout(openTimer); openTimer = null; }
          pauseAudio();
          maxTimer = setTimeout(function () { done({ shown: true, rewarded: rewarded }); }, 180000);
        };
        openTimer = setTimeout(function () {
          if (!opened) done({ shown: false, rewarded: false });
        }, 4500);
        if (type === 'rewarded' && typeof window.ysdk.adv.showRewardedVideo === 'function') {
          window.ysdk.adv.showRewardedVideo({
            callbacks: {
              onOpen: onOpen,
              onRewarded: function () { rewarded = true; },
              onClose: function () { done({ shown: opened, rewarded: rewarded }); },
              onError: function () { done({ shown: false, rewarded: false }); },
            },
          });
        } else {
          window.ysdk.adv.showFullscreenAdv({
            callbacks: {
              onOpen: onOpen,
              onClose: function (wasShown) { done({ shown: !!wasShown || opened, rewarded: false }); },
              onError: function () { done({ shown: false, rewarded: false }); },
            },
          });
        }
...
// Platform-neutral leaderboard wrapper. Yandex's current SDK exposes
// ysdk.leaderboards.setScore/getEntries directly; legacy SDKs used
// ysdk.getLeaderboards().setLeaderboardScore/getLeaderboardEntries.
var _yandexLbPromise = null;
function _yandexLeaderboardApi() {
  if (platform !== 'yandex' || !window.ysdk) return Promise.resolve(null);
  if (window.ysdk.leaderboards) return Promise.resolve(window.ysdk.leaderboards);
  if (typeof window.ysdk.getLeaderboards === 'function') {
    if (!_yandexLbPromise) _yandexLbPromise = window.ysdk.getLeaderboards().catch(function () { return null; });
    return _yandexLbPromise;
  }
  return Promise.resolve(null);
}
function _yandexMethodAvailable(name) {
  try {
    if (window.ysdk && typeof window.ysdk.isAvailableMethod === 'function') {
      return window.ysdk.isAvailableMethod(name).catch(function () { return false; });
    }
  } catch (e) {}
  return Promise.resolve(true);
}
function _formatYandexLeaderboard(result) {
  var entries = result && result.entries ? result.entries : [];
  var out = [];
  for (var i = 0; i < entries.length; i++) {
    var e = entries[i] || {}, p = e.player || {};
    out.push({
      rank: e.rank,
      score: e.score,
      name: p.publicName || p.name || p.uniqueID || 'Player',
      username: p.publicName || p.name || p.uniqueID || 'Player',
    });
  }
  return out;
}
var leaderboardApi = {
  publish: function (tag, score) {
    ...
    if (!tag || score <= 0) return Promise.resolve(false);
    var gp = _gp();
    if (gp && gp.player) {
      try { gp.player.set(tag, score); gp.player.sync(); return Promise.resolve(true); } catch (e) {}
    }
    return _yandexLeaderboardApi().then(function (lb) {
      if (!lb) return false;
      return _yandexMethodAvailable('leaderboards.setScore').then(function (ok) {
        if (!ok) return false;
        if (typeof lb.setScore === 'function') return Promise.resolve(lb.setScore(tag, score)).then(function () { return true; }).catch(function () { return false; });
        if (typeof lb.setLeaderboardScore === 'function') return Promise.resolve(lb.setLeaderboardScore(tag, score)).then(function () { return true; }).catch(function () { return false; });
        return false;
  ...
  fetch: function (tag, limit) {
    ...
    var gp = _gp();
    if (gp && gp.leaderboard && typeof gp.leaderboard.fetch === 'function') {
      try {
        return gp.leaderboard.fetch({ tag: tag, limit: limit })
          .then(function (r) { return (r && r.players) || []; })
          .catch(function () { return []; });
      } catch (e) { return Promise.resolve([]); }
    }
    return _yandexLeaderboardApi().then(function (lb) {
      if (!lb) return [];
      var opts = { quantityTop: limit, includeUser: true, quantityAround: 0 };
      if (typeof lb.getEntries === 'function') return Promise.resolve(lb.getEntries(tag, opts)).then(_formatYandexLeaderboard).catch(function () { return []; });
      if (typeof lb.getLeaderboardEntries === 'function') return Promise.resolve(lb.getLeaderboardEntries(tag, opts)).then(_formatYandexLeaderboard).catch(function () { return []; });
      return [];
    }).catch(function () { return []; });
  },
};
...
window.GF = {
  ...
  gp: gpApi,
  // Platform-neutral leaderboard wrapper. Use this for Yandex/GamePush builds;
  // technical name must match the console leaderboard (for Backrooms: score).
