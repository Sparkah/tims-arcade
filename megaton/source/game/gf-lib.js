...
function platformAdProvider() {
  try {
    if (window.__gfPlatformAds && typeof window.__gfPlatformAds.showAd === 'function') return window.__gfPlatformAds;
  } catch (e) {}
  try {
    if (window.parent && window.parent !== window && window.parent.__gfPlatformAds && typeof window.parent.__gfPlatformAds.showAd === 'function') return window.parent.__gfPlatformAds;
  } catch (e) {}
  return null;
}
function platformAdConfig() {
  var provider = platformAdProvider();
  return provider && provider.config && typeof provider.config === 'object' ? provider.config : null;
}
function requestPlatformAd(type, ok) {
  var provider = platformAdProvider();
  ...
  var finish = function (result) {
    ...
    result = result || {};
    ok({
      shown: !!result.shown || !!result.rewarded,
      rewarded: !!result.rewarded
    });
  };
  try {
    var platformType = type === 'rewarded' ? 'rewarded' : 'interstitial';
    var result = provider.showAd(platformType, function (success, cbResult) {
      finish(cbResult || { shown: !!success, rewarded: platformType === 'rewarded' && !!success });
    });
    Promise.resolve(result).then(finish).catch(function () { finish({ shown: false, rewarded: false }); });
...
function showAd(type) {
  ...
  return new Promise(function(rawOk) {
    ...
    try {
      if (requestPlatformAd(type, ok)) return;
...
function _adsSdkReady(type) {
  try {
    if (platformAdProvider()) return true;
...
try {
  var _platformAdConfig = platformAdConfig();
  if (_platformAdConfig) adsApi.configure(_platformAdConfig);
} catch (e) {}
