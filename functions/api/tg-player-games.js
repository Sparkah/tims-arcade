...
import { publicTelegramUser, readTelegramPlayedGames } from '../_lib/telegramPlayHistory.js';
...
export async function onRequestPost({ request, env }) {
  ...
  const [played] = await Promise.all([
    readTelegramPlayedGames(env, auth.user.id),
  ...
  return json(
    {
      ok: true,
      user: publicTelegramUser(auth.user),
      botProfile: auth.botProfile,
      played,
    },
    200,
