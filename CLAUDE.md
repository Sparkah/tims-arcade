...
- **Direct-upload deploys replace the WHOLE tree — freshness-check first.** Before ANY `wrangler pages deploy` to production, run `bash scripts/predeploy_freshness_check.sh`: the live production Source commit must be an ancestor of your HEAD, else you silently revert another session's shipped fix (2026-07-21: a /cplay worktree deploy reverted the dissertation universal-fit shell 34 min after it went live). If blocked, merge origin/main (or the live commit) and deploy the superset; after deploying a fix, push it to origin/main promptly.
...
- **Free KV tier — 1000 writes/day**. The snapshot cache + visibility-aware heartbeat pattern reduces this, but `heartbeat.js` is still the main write-heavy public path. Don't add ungated writes without checking the budget.
...
- **KV quota runbook**: run `CLOUDFLARE_ACCOUNT_ID=<account> node scripts/kv_usage_report.mjs --days 4 --hourly` before guessing. If same-day usage or projection crosses ~80% of any free cap, pause nonessential writes/lists and tell Tim to upgrade Workers Paid before 00:00 UTC; paid KV changes reads/writes/lists/deletes to monthly allowances. 2026-07-01 pull: only `VOTES` had activity; LIST was controlled after `Gallery#4b49076`, reads were below cap, and writes were the active limiter. Next write-heavy mitigation should move heartbeat public counters/rate buckets to D1 or Analytics Engine before widening traffic/social features.
