...
function drawFloats() {
  for (var i = 0; i < floats.length; i++) {
    ...
    var ffs = clamp(15*S, 11, 22);
    ctx.font = 'bold ' + ffs + 'px sans-serif';
    ctx.fillStyle = f.col; ctx.textAlign = 'center'; ctx.textBaseline = 'alphabetic';
    // 1.10.1: labels spawned near an edge must not bleed off the canvas; if the
    // label is wider than the canvas itself, shrink it until it fits.
    var tw = ctx.measureText(f.txt).width;
    while (tw > W - 10 && ffs > 8) { ffs--; ctx.font = 'bold ' + ffs + 'px sans-serif'; tw = ctx.measureText(f.txt).width; }
