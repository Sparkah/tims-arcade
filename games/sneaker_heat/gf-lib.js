...
function init(config) {
  ...
  if (window.__gpReady && window.gp) {
    ...
    try {
      ...
      //
      // Two INDEPENDENT mute reasons: an open ad (pause/resume) and the platform
      // sound toggle (soundOff/soundOn). Gate on both so soundOn can't un-mute
      // mid-ad, and an ad's resume can't override a platform mute. pauseAudio/
      // resumeAudio stay idempotent; resumeAudio already respects the in-game
      // mute + tab-hidden.
      if (window.GamePix.on) {
        var _gpAd = false, _gpMute = false;
        var _gpApply = function () { if (_gpAd || _gpMute) pauseAudio(); else resumeAudio(); };
        window.GamePix.on.pause    = function () { _gpAd = true;   _gpApply(); };
        window.GamePix.on.resume   = function () { _gpAd = false;  _gpApply(); };
        window.GamePix.on.soundOff = function () { _gpMute = true; _gpApply(); };
        window.GamePix.on.soundOn  = function () { _gpMute = false; _gpApply(); };
      }
    } catch (e) {}
    boot();
    setTimeout(boot, 3000);
