...
function _musSchedule() {
  ...
  while (_mus.next < c.currentTime + 0.25) {
    ...
    if (step === 0) {                                // bar: pad chord + bass + kick
      ...
      if (p.drums && !_mus.noDrums) _musKick(t);
    }
    ...
    if (p.drums && !_mus.noDrums && step % 2 === 1) _musNoise(t, 0.035, 0.045, 7000);  // offbeat hats
...
function startMusic(opts) {
  ...
  _mus.noDrums = !!opts.noDrums;   // strip kick/hats so the game's smashes supply percussion
  _mus.bpmOverride = (typeof opts.bpm === 'number' && opts.bpm > 0) ? opts.bpm : 0;
  _mus.rootOverride = (typeof opts.root === 'number' && opts.root > 0) ? opts.root : 0;   // force key (match game's note root)
  _mus.gainOverride = (typeof opts.gain === 'number' && opts.gain >= 0) ? opts.gain : -1;
  ...
  _mus.root = _mus.rootOverride || roots[((seed >>> 5) & 0xffff) % roots.length];
  ...
  _mus.bpm = _mus.bpmOverride || (p.bpm * (0.92 + (((seed >>> 13) & 0xff) / 255) * 0.16));   // override (game beat sync) or +/-8% tempo
  ...
  var begin = function () {
    ...
    try {
      ...
      _mus.master.gain.value = _mus.muted ? 0.0001 : (_mus.gainOverride >= 0 ? _mus.gainOverride : _mus.preset.gain);
      ...
      _mus.t0 = _mus.next;   // audio-clock origin of the beat grid (for GF.beatInfo)
...
function setMusicMuted(m) {
  _mus.muted = !!m;
  if (_mus.master && _mus.ctx) { try { var ug = (_mus.gainOverride >= 0 ? _mus.gainOverride : (_mus.preset ? _mus.preset.gain : 0.15)); _mus.master.gain.setTargetAtTime(m ? 0.0001 : ug, _mus.ctx.currentTime, 0.04); } catch (e) {} }
}
...
// ── DRUM KIT — percussion synth on the shared SFX context (mute/pause aware).
// For rhythm games where the player's actions ARE the beat: GF.drum('snare'|
// 'kick'|'hat'|'ohat'|'tom'|'clap'|'crash'). Snare/hat/clap/crash use a noise
// buffer; kick/tom are pitched sine drops. Pairs with startMusic({noDrums:true}).
function _noise(t, dur, peak, hp) {
  var c = audioCtx(); if (!c) return;
  var n = Math.max(1, Math.floor(c.sampleRate * dur)), buf = c.createBuffer(1, n, c.sampleRate), d = buf.getChannelData(0);
  for (var i = 0; i < n; i++) d[i] = Math.random() * 2 - 1;
  var src = c.createBufferSource(); src.buffer = buf;
  var g = c.createGain(); g.gain.setValueAtTime(peak, t); g.gain.exponentialRampToValueAtTime(0.0001, t + dur);
  if (hp) { var f = c.createBiquadFilter(); f.type = 'highpass'; f.frequency.value = hp; src.connect(f); f.connect(g); }
  else src.connect(g);
  g.connect(c.destination); src.start(t); src.stop(t + dur + 0.02);
}
function drum(name) {
  if (AUDIO_MUTED || _extPaused) return;
  var c = audioCtx(); if (!c) return; var t = c.currentTime;
  if (name === 'kick') tone(150, 0.18, 'sine', 0.5, 48);
  else if (name === 'snare') { _noise(t, 0.17, 0.22, 1300); tone(190, 0.12, 'triangle', 0.12, 120); }
  else if (name === 'hat') _noise(t, 0.04, 0.10, 8000);
  else if (name === 'ohat') _noise(t, 0.17, 0.07, 8000);
  else if (name === 'tom') tone(185, 0.22, 'sine', 0.32, 95);
  else if (name === 'clap') { _noise(t, 0.02, 0.18, 1500); _noise(t + 0.02, 0.03, 0.18, 1500); _noise(t + 0.05, 0.12, 0.16, 1100); }
  else if (name === 'crash') { _noise(t, 0.55, 0.13, 6000); tone(520, 0.3, 'triangle', 0.05); }
}
// named stinger library — games call GF.sfx('merge') etc.
var SFX_LIB = {
  kick:    function () { drum('kick'); },
  snare:   function () { drum('snare'); },
  hat:     function () { drum('hat'); },
  tom:     function () { drum('tom'); },
  clap:    function () { drum('clap'); },
  crash:   function () { drum('crash'); },
...
window.GF = {
  ...
  sfx: sfx, tone: tone, arp: arp, drum: drum, music: music, toggleMute: toggleMute, drawMuteIcon: drawMuteIcon,
  // Beat clock derived from the procedural bed's AUDIO clock (drift-free) so a
  // rhythm game can lock its beat ring / on-beat scoring to the music. Returns
  // {on, bpm, beatLen, beat, phase} (phase 0..1 within the current quarter-note).
  beatInfo: function () {
    if (!_mus.on || !_mus.ctx || !_mus.t0) return { on: false, bpm: _mus.bpm || 0, beatLen: 0, beat: 0, phase: 0 };
    var beatLen = 60 / (_mus.bpm || 120);
    var elapsed = _mus.ctx.currentTime - _mus.t0;
    var beat = Math.floor(elapsed / beatLen);
    return { on: true, bpm: _mus.bpm, beatLen: beatLen, beat: beat, phase: elapsed / beatLen - beat };
  },
