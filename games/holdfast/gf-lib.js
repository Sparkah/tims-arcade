...
function drawOverlay(opts) {
  ...
  // title with glow
  ctx.shadowColor = 'rgba(0,225,255,0.45)'; ctx.shadowBlur = 30*S;
  ...
  if (opts.btnText && opts.btnFn) {
    ...
    grad.addColorStop(0, '#00e1ff'); grad.addColorStop(1, '#009fd0');
    ctx.save(); ctx.shadowColor = 'rgba(0,225,255,'+(0.4+0.28*_k).toFixed(3)+')'; ctx.shadowBlur = (22+10*_k)*S;
...
// Web Audio bg-music: fetch -> decodeAudioData -> looping BufferSource. NO media element.
...
// Tab-hide / minimize must silence the mp3 loop + SFX (Yandex 1.3). Cross-platform
// fallback for the _ac context; game_api_pause is the authoritative Yandex signal.
(function () {
  document.addEventListener('visibilitychange', function () {
...
function toggleMute() {
  ...
  if (_musicGain) { try { _musicGain.gain.value = AUDIO_MUTED ? 0 : _musicVol; } catch (e) {} }
  try { setMusicMuted(AUDIO_MUTED); } catch (e) {}
  if (!AUDIO_MUTED) { audioCtx(); _startMusicNode(); }
  return AUDIO_MUTED;
}
...
window.GF = {
  ...
  // Clear the declarative overlay click-targets. Games that draw their own
  // in-world UI during play MUST call this each frame they are NOT showing a
  // GF.drawOverlay screen — otherwise the last overlay's button (e.g. PLAY)
  // stays registered and a world tap on its old rect re-fires its fn.
