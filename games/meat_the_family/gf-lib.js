  ...
  if (window.__gpReady && window.gp) {
    ...
    try {
      ...
      if (window.GamePix.on) {
        window.GamePix.on.pause    = function () { _gpAd = true;   _adMuteApply(); };
        window.GamePix.on.resume   = function () { _gpAd = false;  _adMuteApply(); };
...
function showAd(type) {
  ...
  return new Promise(function(rawOk) {
    ...
    var ok = function (res) { if (_resolved) return; _resolved = true; _manualAdOpen = false; _adMuteApply(); rawOk(res); };
...
var _gpAd = false, _gpMute = false, _manualAdOpen = false;
var _gpStopTimer = null, _gpHappyTimer = null; // deferred GamePix lifecycle signals (see gameplayStop)
function _adMuteApply() { if (_gpAd || _gpMute || _manualAdOpen) pauseAudio(); else resumeAudio(); }
function _adOpenPause() { _manualAdOpen = true; pauseAudio(); }
function pauseAudio() {
